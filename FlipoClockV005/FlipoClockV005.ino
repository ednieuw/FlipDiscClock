...
void EverySecondCheck(void)
{
 ...
 // SecondUpdate(TestLDR);                                                                           // Every second an intensitiy check and update from LDR reading 
 if(TestLDR)
   {
    if (Mem.UseDS3231)   sprintf(sptext,"%5lld l/s %0.0f°C ", Loopcounter,RTCklok.getTemperature()); 
...
//  changed functionname DimLeds(bool print)
//--------------------------------------------
void SecondUpdate(bool print) 
{ 
...
void Displaytime()
{ 
DisplayFlipoTime();                                                                                  // Start by clearing the display to a known state   
 ...
 *----------------------------------------------------------------------*/
uint8_t ReadRX8025Ttime(byte *second, byte *minute, byte *hour, byte *dayOfWeek, byte *dayOfMonth, byte *month, byte *year)
{
 ...
 *----------------------------------------------------------------------*/
uint8_t SetRX8025Ttime(byte second, byte minute, byte hour, byte dayOfWeek, byte dayOfMonth, byte month, byte year)
{
  Wire.beginTransmission(RX8025T_ADDR);
  Wire.write((uint8_t)RX8025T_SECONDS);
...
void Demomode(void)
{
 ...
 {    
  ...
  SecondUpdate(false);                                                                         // Set the colour per second of 'IS' and 'WAS' 
  Displaytime();
  Tekstprintln("");
  SerialCheck();
 }

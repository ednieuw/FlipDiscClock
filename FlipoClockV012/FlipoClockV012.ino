/* 
Rename interrupt handler functions:
- void setButtonPressedISR(void) -> void IRAM_ATTR setButtonPressedISR(void)
- void rtcInterruptISR(void) -> void IRAM_ATTR rtcInterruptISR(void)
and move these functions to the beginning of the code before setup, it should work
...
Locate the ELEGANTOTA_USE_ASYNC_WEBSERVER macro in the libraries\ElegantOTA.h file, 
and set it to 1:
#define ELEGANTOTA_USE_ASYNC_WEBSERVER 1
 ...
 Changes V010: FlipDiscSpeed in Mem memory and in menu as option. Corrected display off at 00:00. Renamed DS3231 Functions. Menu update 
 Changes V011: Manual written. request->send_P(200, "text/html" -> request->send(200, "text/html"
               If Display off 3X1 segment still working. ->Turned it off
 Changes V012: Upgraded BLE Start BLE Classes NimBLE --> Version 2.x.x. 
               Print to Flipo one second ahead to correct time lapse               
*/
// =============================================================================================================================
...
struct EEPROMstorage {                                                                       // Data storage in EEPROM to maintain them after power loss
  ...
  byte FlipDiscSpeed    = 10;                                                                 // FlipDiscSpeed
...
void InitFlipo(void)
{
 ...
 // attachInterrupt(digitalPinToInterrupt(B2_PIN), setButtonPressedISR, RISING);
  Flip.Init(D7SEG, D7SEG, D3X1, D7SEG, D7SEG);  //  Attention: do not change! Changing these settings may physical damage the flip-disc displays.  Flip.Init(...); it is the second most important function.
  Flip.Delay(50);                                                                            // Delay effect between flip discs.range: 0 - 100ms, max 255ms
 if(Mem.UseDS3231) Flip.Matrix_7Seg(CLR,R,T,C); 
 else              Flip.Matrix_7Seg(CLR,N,T,P);
 delay(300);
 Flip.Display_3x1(1, 1,0,0);                                                                 // Function allows you to control one, two or three discs of the selected D3X1 display.
}
...
{
  // Copy current timeinfo
  struct tm adjTime = timeinfo;

  // Add one second
  adjTime.tm_sec += 1;
  mktime(&adjTime);  // Normalize (handles minute/hour rollover)

  Flip.Delay(Mem.FlipDiscSpeed); // Delay effect between flip discs. Range: 0–255 ms

  // Extract digits from adjusted time
  digit[0] = (adjTime.tm_hour / 10) % 10;
  digit[1] = (adjTime.tm_hour /  1) % 10;
  digit[2] = (adjTime.tm_min  / 10) % 10;
  digit[3] = (adjTime.tm_min  /  1) % 10;

  Flip.Matrix_7Seg(digit[0], digit[1], digit[2], digit[3]); // Display adjusted time    
  Print_tijd();                                             // Print (actual) time to serial monitor
}

/*
void DisplayFlipoTime(void)
{
  Flip.Delay(Mem.FlipDiscSpeed);                                                           // Delay effect between flip discs.range: 0 - 100ms, max 255ms
  digit[0] = (timeinfo.tm_hour / 10) % 10;                                                    // Extract individual digits for the display
  digit[1] = (timeinfo.tm_hour /  1) % 10;
  digit[2] = (timeinfo.tm_min  / 10) % 10;
  digit[3] = (timeinfo.tm_min  /  1) % 10;
  Flip.Matrix_7Seg(digit[0],digit[1],digit[2],digit[3]);                                      // Display the current time    
  Print_tijd();                                                                               // Print time to the serial monitor
}
*/
//--------------------------------------------                                                //
//  FLIPO
//  Turn On OFF Flipo 3x1 Display 
//--------------------------------------------
void SetFlipoSeconds(bool Toggle)
{
...
void setup() 
{
 ...
 InitDS3231Mod();                       Tekstprintln("RTC module software started");          // Start the DS3231 RTC-module even if not installed. It can be turned it on later in the menu
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
   {
    ...
    if(!LEDsAreOff) SetFlipoSeconds(Toggle);                                                  // Set 3x1 Display
    if(TestLDR)                                                                               // If readings every second in serial terminal
     {
...
// sprintf(sptext,"Password: %s", Mem.Password);                                               Tekstprintln(sptext);
 ...
 sprintf(sptext,"Disc speed: %d ", Mem.FlipDiscSpeed);                                         Tekstprintln(sptext); 
 sprintf(sptext,"Time from: %s", Mem.UseDS3231?"RTCmod":"NTP");                                Tekstprintln(sptext);                                  
  ...
  switch (InputString[0])
   {
    ...
    case 'Q':
             sprintf(sptext,"**** Length fault Q. ****");         
              if (InputString.length() < 5 )
               {
                Mem.FlipDiscSpeed = (byte) SConstrainInt(InputString,1,4,0,255);
                sprintf(sptext,"FlipDisc speed: %d ms", Mem.FlipDiscSpeed);                
               }
             break;
    case 'R':
    case 'r':
...
/*/-----------------------------
// BLE Start BLE Classes Version 1.x.x
//------------------------------
